<template>
  <div>
    ...
    <div style="margin-top: 4px; color: #1D2129; font-weight: bold; text-align: left; font-size: 18px; margin-left: 16px; float: left">非结构化导入</div>
    <div style="">
      <div style="margin-top: 150px; width: 500px; text-align: left; margin-left: calc(50% - 250px)" v-show="this.active">
      ...
      <div v-show="!this.active">
        <img src="~@/assets/success.png" style="height: 250px; margin-top: 20px; clear: both; display: block; margin: auto;" />
        <div style="font-size: 24px; color: #2A82E4">提交完成，已提交非结构化数据，请在日志中查看。</div>
      </div>
...
export default {
  ...
  methods:  {
    ...
    handleChange(file, fileList){
      console.log(file)
      let isZip = file.name.split('.')[1] === 'txt'
      if (!isZip) {
        this.$message.error({
          message: '只能上传.txt文件'
        });
