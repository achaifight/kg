  ...
  <div>
    <div v-if="!this.create">
      ...
      <div style="margin-top: 4px; display: block; overflow: hidden">
        ...
        <div style="display: inline; float: right; height: 32px">
          ...
          <el-button type="primary" style="margin-left: 18px; background: #1C74F1; font-size: 14px; font-weight: bold" size="mini" @click="this.createNew">新建</el-button>
        </div>
      </div>
    ...
    <div v-else-if="this.create">
      ...
      <div v-show="this.steps==1">
        ...
        <div style="margin-top: 50px; width: 560px; text-align: left; margin-left: calc(50% - 280px)">
          ...
          <div style="display: inline; width: 350px">
            <el-input :disabled = "true" v-model = "file.zip" style="width: 350px">
      ...
      <div v-show="this.steps == 4">
        ...
        <div style="font-size: 24px; color: #2A82E4">已完成模板映射，请点击确认提交，请在任务管理中使用该模板。</div>
      </div>
      ...
      <div>
        <el-dialog title="关系映射" :visible.sync="relationRow.dialogVisible" width="40%" :before-close="handleClose">
          <el-row>
            ...
            <el-col :span="10">
              <el-select v-model="relationRow.rowData.relationFile" @change="handleRelationFile">
                <el-option
...
export default {
  ...
  methods:{
    ...
    identityRelationRow(){
      console.log(this.relationRow.rowData)
    ...
    handleRelationFile(){
      getJsonMappingSituation(this.relationRow.rowData.relationFile, this.this_file.zip).then(response => {
    ...
    submitUpload() {
      if (this.steps === 1){
        ...
        handlingOpenData(this.this_file.zip, this.this_file.schemaName, 2).then(response => {
          console.log(response.data)
          this.relationTable = response.data
          console.log(this.relationTable)
          this.steps = 1;
  ...
  created() {
    // window.handleCsvPackageDownload = this.handleCsvPackageDownload
  }
}
</script>
